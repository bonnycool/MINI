...
const AdminOSEvents = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            const docRef = await addDoc(collection(db, 'openevents'), {
                ...formData,
    ...
    const handleApproveRegistration = async (regId, eventId) => {
        try {
            const regDocRef = doc(db, 'open-event-reg', regId);
    ...
    useEffect(() => {
        ...
        const fetchRegistrations = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, 'open-event-reg'));
