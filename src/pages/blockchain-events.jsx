...
const BlockchainEvents = () => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, 'blockevents'));
        ...
        const fetchRegisteredEvents = async (uid) => {
            try {
                const q = query(collection(db, 'blockeventreg'), where('uid', '==', uid));
   ...
   const handleRegister = async (eventId) => {
    ...
    if (user) {
        ...
        try {
            ...
            const eventDoc = await getDoc(doc(db, 'blockevents', eventId));
            if (eventDoc.exists()) {
...
const handleApproval = async (registrationId, eventId) => {
    try {
        const regDocRef = doc(db, 'blockeventreg', registrationId);
    ...
    return (
        <div className="flex h-screen">
            ...
            <div className="flex-1 p-8 bg-gray-100">
                <UserHeader />
                <h2 className="text-3xl font-bold mt-6 text-gray-800">Upcoming Events</h2>
