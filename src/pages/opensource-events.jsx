...
const OSEvents = () => {
    ...
    useEffect(() => {
        ...
        const fetchRegisteredEvents = async (uid) => {
            try {
                const q = query(collection(db, 'open-event-reg'), where('uid', '==', uid));
   ...
   const handleRegister = async (eventId) => {
    ...
    if (user) {
        ...
        try {
            ...
            const eventDoc = await getDoc(doc(db, 'openevents', eventId));
            if (eventDoc.exists()) {
...
const handleApproval = async (registrationId, eventId) => {
    try {
        const regDocRef = doc(db, 'open-event-reg', registrationId);
